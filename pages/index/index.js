...
Page({
  ...
  onLoad: function(options) {
    var that = this
    // wx.navigateTo({
    //   url: '/pages/personal/integral/integralmall/detail',
    // })
    app.api.system((info) => {
      that.setData({
        system: info,
    ...
    if (wx.createRewardedVideoAd) {
      videoAd = wx.createRewardedVideoAd({
        adUnitId: 'adunit-f9d5729bd143ebb6'
      })
      videoAd.onLoad(() => {})
      videoAd.onError((err) => {})
      videoAd.onClose((res) => {})
  ...
  seleCity(renew) {
    ...
    new Promise((resolve, reject) => {
      ...
      for (var i = 0; i < res.page.length; i++) {
        if (res.page[i].cmpName == 'flow') {
          wx.setStorageSync('codeAd', res.page[i].config.code)
          break;
        } else {
          wx.setStorageSync('codeAd', false)
        }
      };
  ...
  onReachBottom: function() {
    var selecttype = this.data.selecttype,
      ...
      page = this.data.page;
    // console.log('onReachBottom', selecttype, postList, page)
  ...
  onShareAppMessage: function(res) {
    var that = this,
      ...
      imageUrl = that.data.url + system.shareImg;
    ...
    if (system.shareImg != '') {
      return {
        imageUrl: system.shareImg,
        title: `【${system.title}】${system.shareDescription}`,
      }
    } else {
      return {
        title: `【${system.title}】${system.shareDescription}`,
