...
Page({
  ...
  onLoad: function (options) {
    ...
    app.api.request({
      ...
      data: { id: wx.getStorageSync('users').id, type:1},
      success: res => {
        console.log(res)
        that.setData({
    ...
    app.api.identSet((info) => {
      ...
      if (info.bondPersonal == '1') {
        arr.push({ bond: '1', name: '个人基本保证金' + info.personalMoney + '元', money: info.personalMoney,})
      }
      that.setData({
        identSet: info,
        actions:arr,
  ...
  handleClickItem({
    ...
    app.api.request({
      ...
      'method': 'POST',
      data: {
        userId: that.data.userinfo.id, money: typeinfo.money, type: 1,
      },
      success: function (res) {
        wx.hideLoading()
        if (res.data.code == '1') {
