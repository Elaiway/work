Component({
  externalClasses: ['extra-class'],
    ...
    methods: {
         ...
         */
        onSelect() {
          var that = this, count = this.data.count - this.data.fileList.length;
          console.log(count)
            const { sizeType, sourceType, uploaded, disabled } = this.data
            const success = (res) => {
                this.tempFilePaths = res.tempFilePaths.map((item) => ({ url: item, uid: this.getUid() }))
              this.setData({
                fileList: that.data.fileList.concat(this.tempFilePaths),
              })
              const info = {
                fileList: that.data.fileList,
              }
              this.onChange(info)
              //this.triggerEvent('before', that.data.fileList)
         ...
         */
        onRemove(e) {
          const { file } = e.currentTarget.dataset, { index } = e.currentTarget.dataset
            const fileList = [...this.data.fileList]
            //const index = fileList.map((item) => item.uid).indexOf(file.uid)
