...
Page({
  ...
  tzxq(e) {
    ...
    if (index == 0) {
      if (rzarr[index].typename == '未认证') {
        wx.navigateTo({
          url: 'testcompany?storeid=' + this.data.storeid,
        })
    ...
    if (index == 1) {
      wx.navigateTo({
        url: 'sjtestbond?storeid=' + this.data.storeid,
      })
      return
  ...
  onLoad: function(options) {
    ...
    //app.pageOnLoad(this);
    app.setNavigationBarTitle('商家认证中心')
    that.setData({
      Swiper: {
        "padding": 0,
        "height": 200,
  ...
  onShow: function() {
    ...
    app.api.identSet((info) => {
      info.identType.forEach(function(item, index) {
        if (info.identOpen == '1' && item == 2) {
          rzarr[0].show = true
        }
      })
      if (info.bondOpen == '1' && info.bondCompany == '1') {
        rzarr[1].show = true
      }
