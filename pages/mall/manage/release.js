...
Page({
  ...
  onLoad: function(options) {
    ...
    app.setNavigationBarTitle('发布商品')
    let uploadArr = [{
        tips: '上传商品主图，最多上传3张',
        title: '',
        count: '3',
    ...
    //商家分类
    app.api.prequest({
      'url': app.urlTwo.shopCategoryList,
    }).then(res => {
      var hyarr = res.data;
      var hyMultiArray = [
        [...hyarr],
  ...
  getSpec() {
    app.api.prequest({
      'url': app.urlTwo.shopGoodsInfo,
      data: {
        goodsId: this.data.params.id
      },
    }).then(res => {
      this.setData({
        parentSpec: res.data.spec,
        parentGroup: res.data.group,
      })
    })
  },
  //公用change事件
  ...
  goSpec() {
    wx.navigateTo({
      url: 'releasespec?parentSpec=' + JSON.stringify(this.data.parentSpec) + '&parentGroup=' + JSON.stringify(this.data.parentGroup) +'&goodsId='+this.data.params.id,
    })
  },
  postSwitchChange(e) {
    this.setData({
  ...
  formSubmit: function(e) {
    ...
    for (let k in v) {
      ...
      if (k == 'isSpec' || k == 'isPost') {
        params[k] = v[k] ? '1' : '2'
      }
    }
    //根据开关添加过滤字段
    if (!v.isSpec || co.isEdit) {
      filterArr.push('spec')
    }
    if (v.isPost) {
    ...
    let judgeData = app.com.isFailParams({
      field: params,
      filter: ['id', 'category', 'checkbox', 'detailImgs', 'group'].concat(filterArr),
      tips: {
        storeId: '缺少商家id',
        typePid: '请选择商品分类',
        typeId: '请选择商品二级分类',
    ...
    if (!judgeData) {
      return
    } else if (v.isSpec && co.isEdit && !co.parentSpec.length) {
      warn = '请添加商品规格'
    } else {
      //需要改变数据格式
      if (co.isEdit) {
        params.spec = params.group = []
      }
      params.spec = JSON.stringify(params.spec)
      params.group = JSON.stringify(params.group)
